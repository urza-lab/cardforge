...
Proxmox LXC (Debian, Docker + Compose v2). `docker compose up -d --build`
brings up all services; `/api/health/ready` and the System Status page both
report postgres/redis OK.

Repo: `https://github.com/urza-lab/cardforge` (public). Tags `v0.1.0-phase1`
through `v0.1.3-phase1` mark the incremental fixes described below.

**Next up: Phase 2** (DB models, Alembic migrations, collection import —
ManaBox CSV, generic CSV, text lists, import preview/errors). See
`ARCHITECTURE.md` for the full 7-phase plan.
